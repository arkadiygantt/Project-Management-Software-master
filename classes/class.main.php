...
class Main extends Database {
    ...
    private function loadLanguage() {
        ...
        if (isset($_SESSION['logged']) || isset($_COOKIE['logged'])) {
            if (isset($_SESSION['logged']) && is_array($_SESSION['logged']) && isset($_SESSION['logged']['id'])) {
                $user_id = $_SESSION['logged']['id'];
            } elseif (isset($_COOKIE['logged'])) {
                $cookie_data = unserialize($_COOKIE['logged']);
    ...
    public function get_alert() {
        ...
        // Возвращаем значение свойства или пустую строку, если оно не задано
        return $this->alert ?? '';
    }
